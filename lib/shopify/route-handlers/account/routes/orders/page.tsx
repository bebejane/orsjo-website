				...
				{orders.map((order, idx) => {
					return (
						<tbody key={idx} className={order.canceledAt ? s.cancelled : undefined}>
							<tr>
								<td colSpan={2}>{order.processedAt}</td>
							...
							{order.lineItems.edges.map((lineItem, i) => (
								<tr key={i}>
									...
									<td>{lineItem.node.title}</td>
									<td>{lineItem.node.variant?.title}</td>
									<td>
...
export const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
	...
	const { customer }: { customer: Customer } = await shopifyQuery(CustomerOrdersDocument, {
