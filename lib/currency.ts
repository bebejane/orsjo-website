...
export const getAllCurrencyRates = async (revalidate?: number): Promise<CurrencyRate[]> => {
...
export const getCurrencyRateByLocale = async (
	locale: SiteLocale | string,
	revalidate?: number,
): Promise<CurrencyRate> => {
	const { currency } = await apiQuery(CurrencyDocument, {
		revalidate: revalidate ?? parseInt(process.env.REVALIDATE_TIME!),
