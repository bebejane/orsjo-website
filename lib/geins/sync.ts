...
import geinsQuery from '@/lib/shopify/shopify-query';
	...
	try {
		...
		switch (apiKey) {
			case 'product':
				...
				const { product: shopifyProduct } = await geinsQuery(AdminProductDocument, {
					admin: true,
			...
			case 'product_accessory':
				...
				const { product: shopifyAccessory } = await geinsQuery(AdminProductDocument, {
					admin: true,
				...
				const accessoryVariants: ProductVariantsBulkInput[] = [
					{
						id: shopifyAccessory?.variants.edges.find(
			...
			case 'product_lightsource':
				...
				const { product: shopifyLightsource } = await geinsQuery(AdminProductDocument, {
					admin: true,
				...
				const lightsourceVariants: ProductVariantsBulkInput[] = [
					{
						id: shopifyLightsource?.variants.edges.find(
...
export async function updateProduct(
	...
	try {
		...
		if (variants) {
			...
			console.log(
				`https://admin.shopify.com/store/orsjo-shop/products/${product.id?.split('/').pop()}`,
			);
		}
	} catch (e) {
		console.log(e);
...
export const resetAll = async () => {
	...
	const { products } = await geinsQuery(AllAdminProductsDocument, {
		admin: true,
	});
	...
	await batchPromises(
		products?.edges.map(
			({ node: { id } }) =>
				() =>
					geinsQuery(RemoveProductDocument, {
						admin: true,
						variables: {
							id,
						},
	...
	while (true) {
		const { files } = await geinsQuery<FilesQuery, FilesQueryVariables>(FilesDocument, {
			admin: true,
			variables: {
				first,
