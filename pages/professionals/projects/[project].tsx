	...
	return (
		<>
			...
			{(relatedProducts.length > 0 || relatedProjects.length > 0) && (
				<Section className={styles.related} name={'Related'} bgColor={'--mid-gray'} fadeColor={'--gray'}>
...
export async function getStaticPaths(context) {
	...
	const paths = allProjects.map(({ slug }) => ({ params: { project: slug } }));
...
export const getStaticProps = withGlobalProps({}, async ({ props, context, revalidate }) => {
	const { project, bespokeThumbnail }: { project: ProjectRecord; bespokeThumbnail: BespokeThumbnailRecord } =
		await apiQuery([ProjectDocument, BespokeThumbnailDocument], { variables: { slug: context.params.project } });

	if (!project) return { notFound: true, revalidate };

	const { allProjects }: { allProjects: ProjectRecord[] } = await apiQuery(AllRelatedProjectsDocument, {
		variables: { projectType: project.projectType.id },
	});
	const relatedProjects = allProjects.filter((p) => p.id !== project.id).sort((a, b) => (Math.random() > 0.5 ? 1 : -1));
