...
export const sync = async (itemId: string): Promise<Item> => {
	try {
		let item: Item;
		...
		await updateProduct(products, markets);
		return item;
	} catch (e) {
		console.log(e);
		throw e;
...
export async function updateProduct(p: ProductData[], markets: Market[]) {
...
export const syncProductsPrices = async (products: { ProductId: number; price: number }[]) => {
	const allCurrencies = await getAllCurrencyRates(0);
...
export const resyncAllProductPrices = async () => {
	...
	return { skipped, response, prices, duration: Date.now() - now };
};
