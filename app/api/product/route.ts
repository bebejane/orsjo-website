	...
	return basicAuth(req, async (req) => {
		try {
			...
			if (!productIds.length)
				return NextResponse.json({ success: false, message: 'Invalid Id' }, { status: 200 });

			waitUntil(run(productIds));

			return NextResponse.json({
				success: true,
				message: 'generating product pdfs',
				productIds,
			});
		} catch (err) {
			console.log(err);
...
async function run(productIds: string[]) {
